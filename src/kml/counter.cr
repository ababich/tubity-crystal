class Counter
  ...
  #database keys of global counters
  KEY = "tubity:counter"

  def self.get_hash(redis)
    hash = redis.get(KEY)

    new_hash = next_hash(hash)
    redis.set(KEY, new_hash)

    new_hash
  end


  def self.next_hash(current : Nil)
    (0..1).map{|i| ALPHABET[i][0]}.join
  end

  def self.next_hash(current : String)
    new = current.split(//)
    ...
    while idx >= 0
      last_chr = new[idx]
      last_pos = ALPHABET[idx].index(last_chr)   || -1

      if last_pos < a_size
        new[idx] = ALPHABET[idx][last_pos + 1].to_s

        return new.join
      else
        new[idx] = ALPHABET[idx][0].to_s
        idx -= 1
      end
    end

    new.join + ALPHABET[new.size][0].to_s
  end
end
